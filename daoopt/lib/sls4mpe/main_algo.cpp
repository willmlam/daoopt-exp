...
//last sol taken to be the best! assignmentManager->recordNewBestSolution();
	...
	if (algo != ALGO_HYBRID) { // access violation when algo == ALGO_HYBRID
		int var;
		for (var = 0; var < num_vars; var++){
...
void runAlgorithm(int **outBestAssignment, double *outLogLikelihood, bool& m_solution_found, int& m_num_trial,
	double& m_time_elapsed, double& converge_rate){

	if (preprocessingSizeBound > 0) outputBestMPE = false;
	m_solution_found = false;
	m_num_trial = 1; // by default try at least once
	m_time_elapsed = 0.0; // record global time for conveience

	global_abort = false; // when this is true, while loop stops but we don't use this yet

	/* sls4mpe initialization except creation of assingmnet manager, problem reader */
	mbeElim = new MiniBucketElimination();
	first_init();
	initFromVarsAndProbTables();
	...
	while ((!global_abort) && !abort_flag && num_run < maxRuns && !m_solution_found) { // maxRuns set to 1 or more
	//while ((!global_abort) && !abort_flag && num_run < maxRuns) { // maxRuns set to 1 or more
#ifdef _DEBUG
		std::cout << " sls debug run : " << m_num_trial << std::endl;
#endif
		init_run();
		gls(converge_rate);
		end_run();

		if (assignmentManager->overallBestLogProb == -DOUBLE_BIG) {
			m_solution_found = false;
			m_num_trial++;
		}
		else {
	...
	m_time_elapsed = overall_time_so_far;
...
bool lsContinueInc(double converge_rate){
#ifdef _DEBUG
	std::cout << "run_time_so_far : " << run_time_so_far << std::endl;
	std::cout << "run_time_so_far - assignmentManager->runBestTime : " <<  run_time_so_far - assignmentManager->runBestTime << std::endl;
	std::cout << "converge_rate * assignmentManager->runBestTime : " << converge_rate * assignmentManager->runBestTime << std::endl;
#endif

	return (!global_abort) && run_time_so_far < maxTime
		&& num_flip < maxSteps
		&& assignmentManager->runBestLogProb + EPS < assignmentManager->optimalLogMPEValue
		&& ((run_time_so_far - assignmentManager->runBestTime) < converge_rate * assignmentManager->runBestTime);
	// 1. not max number of flips
	// 2. sub optimal value
	// 3. didn't improve < rate * recent_improvement
}

void gls(double converge_rate){
	instantiation inst;
	if (verbose) printf("starting GLS at time %lf\n", run_time_so_far);
	long lmCounter = 0;
	while (lsContinueInc(converge_rate)) {
		inst = best_new_inst(false);
		//		printf("%d->%d\n",inst.var,inst.value);
		if (inst.var == NOVALUE){
			//		printf("LM %d at step %d, ll=%lf, increasing penalties\n", lmCounter+1, num_flip, log_prob);
