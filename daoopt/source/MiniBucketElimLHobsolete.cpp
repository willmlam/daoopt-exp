...
void MiniBucketElimLHobsolete::reset(void)
{
	for (MiniBucketElimLHobsoleteError & be_error : _LookaheadResiduals) 
		be_error.Delete();
	for (MiniBucketElimLHobsoleteheuristic & be_lh : _Lookahead) 
		be_lh.Delete();
	deleteLocalErrorFNs();
...
/*void MiniBucketElimLHobsolete::Delete(void) 
{
...
*/
size_t MiniBucketElimLHobsolete::build(const std::vector<val_t> *assignment, bool computeTables)
{
...
double MiniBucketElimLHobsolete::getHeurPerIndSubproblem(int var, std::vector<val_t> & assignment, SearchNode *search_node, double label, std::vector<double> & subprobH) 
{
/*// DEBUGGG
...
double MiniBucketElimLHobsolete::getHeur(int var, std::vector<val_t> & assignment, SearchNode *search_node) 
{
#if defined DEBUG || _DEBUG
	int nSet = 0;
...
if (dTest > 1.0e-6) {
	...
	MiniBucketElimLHobsoleteheuristic & lhHelper = _Lookahead[var] ;
...
void MiniBucketElimLHobsolete::getHeurAll(int var, vector<val_t> &assignment, SearchNode *search_node, vector<double> &out)
{
...
//  ++nd1GeneralCalls;
	...
	MiniBucketElimLHobsoleteError & eh = _LookaheadResiduals[var] ;
...
double MiniBucketElimLHobsolete::getLocalError(int var, vector<val_t> & assignment)
{
...
void MiniBucketElimLHobsolete::getLocalError(int parent, int var, vector<val_t>& assignment, std::vector<double>& out)
{
	// check if number of MBs is 1; if yes, bucket error is 0.
	vector<MiniBucket>& minibuckets = _MiniBuckets[var];
...
int MiniBucketElimLHobsolete::computeLocalErrorTable(int var, bool build_table, bool sample_table_if_not_computed, double TableMemoryLimitAsNumElementsLog, double &TableSizeLog, double &avgError, double &avgExact, Function *&errorFn, int64 & nEntriesGenerated) 
{
	nEntriesGenerated = 0 ;
...
#if defined DEBUG || _DEBUG
	if (NULL != m_options ? NULL != m_options->_fpLogFile : false) {
		fprintf(m_options->_fpLogFile,"\n   MiniBucketElimLHobsolete::computeLocalErrorTable var=%d tablesize=%lld",(int)var, (int64)TableSize);
		}
	printf("\n   MiniBucketElimLHobsolete::computeLocalErrorTable var=%d tablesize=%lld", (int)var, (int64)TableSize);
#endif 
...
int MiniBucketElimLHobsolete::computeLocalErrorTables(bool build_tables, double TotalMemoryLimitAsNumElementsLog, double TableMemoryLimitAsNumElementsLog)
{
	vector<int> elimOrder;    // will hold dfs order
	findDfsOrder(elimOrder);  // computes dfs ordering of relevant subtree
	...
	for (vector<int>::reverse_iterator itV = elimOrder.rbegin();itV != elimOrder.rend(); ++itV) {
		...
		if (NULL != errorFn ? NULL != errorFn->getTable() : false) {
			...
			//				fprintf(m_options->_fpLogFile, "\n
			//MiniBucketElimLHobsolete::localError var=%d, avgError=%g avgExact=%g", v, e, E)
			//;
