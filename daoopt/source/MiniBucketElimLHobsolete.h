...
#ifndef MiniBucketElimLH_H_obsolete_
#define MiniBucketElimLH_H_obsolete_
...
class MiniBucketElimLHobsolete ;
class MiniBucketElimLH ;
class MiniBucketElimLHobsoleteErrorNode ;
class MiniBucketElimLHobsoleteError ;
class MiniBucketElimLHobsoleteheuristicNode ;
class MiniBucketElimLHobsoleteheuristic ;

/* enhanced minibucket elimination */
class MiniBucketElimLHobsolete : public MiniBucketElim
{
	friend class MiniBucketElimLHobsoleteError ;
	friend class MiniBucketElimLHobsoleteErrorNode ;
	friend class MiniBucketElimLHobsoleteheuristic ;
	friend class MiniBucketElimLHobsoleteheuristicNode ;
...
public :
	...
	MiniBucketElimLHobsolete(Problem *p, Pseudotree *pt, ProgramOptions *po, int ib) ;
	void printExtraStats() const;
	virtual ~MiniBucketElimLHobsolete(void) ;
} ;

inline MiniBucketElimLHobsolete::MiniBucketElimLHobsolete(Problem* p, Pseudotree* pt, ProgramOptions* po, int ib) :
...
inline void MiniBucketElimLHobsolete::printExtraStats() const {
...
inline MiniBucketElimLHobsolete::~MiniBucketElimLHobsolete(void)
{
  this->reset();
}
...
public :
	...
	std::vector<MiniBucketElimLHobsoleteErrorNode *> _Children ; // children of this node in the bucket tree, that have non-0 bucket error within LH-subteee rooted at this node.
public :
	int Delete(void) 
	{
		for (vector<MiniBucketElimLHobsoleteErrorNode *>::iterator it = _Children.begin(); it!=_Children.end(); ++it) 
			delete *it ;
		_Children.clear() ;
...
/*		int i ;
		...
		if (_Children.size() > 0) {
			vector<MiniBucketElimLHobsoleteErrorNode *>::const_iterator itC_end = _Children.end() ;
			for (i = 0 ; i < _k ; ++i) {
				assignment[_v] = i ;
				for (vector<MiniBucketElimLHobsoleteErrorNode *>::const_iterator itC = _Children.begin() ; itC != itC_end ; ++itC) 
	...
	MiniBucketElimLHobsoleteErrorNode(void) : _H(NULL), _v(-1), _idxWRTparent(-1), _k(-1), _depth2go(-1)
	{
	}
	~MiniBucketElimLHobsoleteErrorNode(void)
	{
		Delete() ;
	}
} ;
...
public :
	int _v ; // variable of this node.
	int _depth ;
	MiniBucketElimLHobsoleteErrorNode _RootNode ;
	std::vector<MiniBucketElimLHobsoleteErrorNode *> _SubtreeNodes ;
public :
	bool IsSubtreeVariable(int v)
	{
		for (vector<MiniBucketElimLHobsoleteErrorNode *>::iterator itRB = _SubtreeNodes.begin(); itRB!=_SubtreeNodes.end(); ++itRB) {
			if (v == (*itRB)->_v) 
				return true ;
			}
		return false ;
	...
	{
		for (vector<MiniBucketElimLHobsoleteErrorNode *>::iterator itRB = _SubtreeNodes.begin(); itRB!=_SubtreeNodes.end(); ++itRB) {
			MiniBucketElimLHobsoleteErrorNode *n = *itRB ;
			int u = n->_v ;
	...
	inline double Error(vector<val_t> & assignment) const
	{
		...
		double e = ELEM_ONE ;
		vector<MiniBucketElimLHobsoleteErrorNode *>::const_iterator itC_end = _RootNode._Children.end() ;
		for (vector<MiniBucketElimLHobsoleteErrorNode *>::const_iterator itC = _RootNode._Children.begin() ; itC != itC_end ; ++itC) 
...
public :
	int Initialize(MiniBucketElimLHobsolete & H, int v, int depth)
	{
		Delete() ;
...
//		std::deque<MiniBucketElimLHobsoleteErrorNode *> nodes2process ;
		std::stack<MiniBucketElimLHobsoleteErrorNode *> nodes2process ; // using stack will turn it into a DFS processing of the tree
		try { nodes2process.push(&_RootNode) ; } catch (...) { return 1 ; }
		int rootchilddepth = depth-1 ;
		MiniBucketElimLHobsoleteErrorNode *currentrootchild = NULL ;
		while (nodes2process.size() > 0) {
			MiniBucketElimLHobsoleteErrorNode *N = nodes2process.top() ; nodes2process.pop() ;
			...
			for (vector<PseudotreeNode*>::const_reverse_iterator itC = children.rbegin() ; itC != children.rend(); ++itC, --idxChild) {
				...
				MiniBucketElimLHobsoleteErrorNode *n = NULL ; try { n = new MiniBucketElimLHobsoleteErrorNode ; } catch (...) { return 1 ; } if (NULL == n) return 1 ; n->_H = &H ; n->_v = child ; n->_k = (H.m_problem)->getDomainSize(child) ; n->_depth2go = depth2go ;
				...
				MiniBucketElimLHobsoleteErrorNode *currentrootchild_ = (NULL == currentrootchild) ? n : currentrootchild ; 
...
this code has bug; for all MB output FNs we should check if it is in [v] bucket first
	...
	MiniBucketElimLHobsoleteError(void) : _H(NULL), _v(-1), _depth(-1)
	{
	}
	~MiniBucketElimLHobsoleteError(void)
	{
	}
} ;
...
public :
	...
	std::vector<MiniBucketElimLHobsoleteheuristicNode *> _Children ; // children of this node in the bucket tree, that have non-0 bucket error within LH-subteee rooted at this node.
public :
	int Delete(void) 
	{
		for (vector<MiniBucketElimLHobsoleteheuristicNode *>::iterator it = _Children.begin(); it!=_Children.end(); ++it) 
			delete *it ;
		_Children.clear() ;
		_RelevantBucketFunctions.clear() ;
		return 0 ;
	...
	{
		vector<Function*>::const_iterator itF_end = _RelevantBucketFunctions.end() ;
		vector<MiniBucketElimLHobsoleteheuristicNode *>::const_iterator itC_end = _Children.end() ;
		double valueMax = ELEM_ZERO ;
#ifdef DEBUG
    bool assign_is_none = false;
#endif
      ...
      if (assignment[_v] == NONE) {
        assign_is_none = true;
			...
			for (vector<Function*>::const_iterator itF = _RelevantBucketFunctions.begin() ; itF != itF_end ; ++itF) 
				value OP_TIMESEQ (*itF)->getValue(assignment) ;
			for (vector<MiniBucketElimLHobsoleteheuristicNode *>::const_iterator itC = _Children.begin() ; itC != itC_end ; ++itC) {
				value OP_TIMESEQ (*itC)->ExactBucketValue(assignment) ;
...
done_this_iteration : 
	...
	MiniBucketElimLHobsoleteheuristicNode(void) : _H(NULL), _v(-1), _idxWRTparent(-1), _k(-1), _depth2go(-1)
	{
	}
	~MiniBucketElimLHobsoleteheuristicNode(void)
	{
		Delete() ;
	}
} ;
...
{
public :
	MiniBucketElimLHobsolete *_H ; // MBE heuristic
public :
	int _v ; // variable of this node.
	int _depth ; // depth of the subtree
	std::vector<MiniBucketElimLHobsoleteErrorNode *> _SubtreeNodes ; // variables/buckets in the (minimal) subtree
	MiniBucketElimLHobsoleteErrorNode _RootNode ; // node corresponding to _v
...
this code has bug; for all MB output FNs we should check if it is in [v] bucket first
	...
	MiniBucketElimLHobsoleteheuristic(void) : _H(NULL), _v(-1), _depth(-1)
	{
	}
	~MiniBucketElimLHobsoleteheuristic(void)
	{
	}
} ;
...
#endif // MiniBucketElimLH_H_obsolete_
