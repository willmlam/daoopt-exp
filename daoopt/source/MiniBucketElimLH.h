...
class MiniBucketElimLHStatistics {
 public:
  int _iBound;
  int _Width;  // num of vars when only original functions placed in the bucket
               // are considered
  int _PseudoWidth;  // num of vars when all functions (original and MB
                     // generated) placed in the bucket are considered (this is
                     // the scope of bucket fn).
  int _EnforcedPseudoWidth;  // "pseudowidth" based on the local error functions
  ...
  vector<size_t> _NumNodesLookahead;  // actual number of lookahead nodes during
                                      // search by variable.
  vector<size_t> _NumNodesLookaheadSkipped;  // actual number of lookahead nodes
                                             // during search by variable for
                                             // those with nonempty lookahead
                                             // subtrees
...
class MiniBucketElimLH : public MiniBucketElim {
 ...
 protected:
  ...
  // in the bucket tree, distance of the variable from the root, as the number
  // of edges needed to travel to reach the bucket. 0 for the root.
  std::vector<int> _Depth;
  int _MaxDepth;

  // joint scope of each bucket, including the bucket's var
  std::vector<std::set<int>> _BucketScopes;
  // original + augmented function of each bucket; this is the set of functions
  // that minibucket algorithm works with.
  ...
  // avg abs of bucket error. when initialized this is -infinity (unknown).
  // actual value should be >=0.
  // avg does not include those error entries that are infinite.
  std::vector<double> _BucketError_AbsAvg, _BucketError_AbsMin,
      _BucketError_AbsMax;
  ...
  // for each var, distance to the closest descendant with more than 1
  // MB/BucketErrorQuality>0. INT_MAX means infinite (no descendants).
  // this does not include the node itself; i.e. legal values are >0 & <=
  // INT_MAX (meaning infinity = no such descendants).
  // note that _distToClosestDescendantWithMBs[i] <=
  // _distToClosestDescendantWithLE[i], since the only way a bucket error can
  // occur
  // is if nMBs>1 (the other way is not neccessarily correct - it could be that
  // nMBs>1, but LE=0).
 ...
 public:
  ...
  inline std::vector<signed char> &BucketErrorQuality(void) {
    return _BucketErrorQuality;
  }
  inline signed char BucketErrorQuality(int v) {
    return _BucketErrorQuality[v];
  }
  inline int distToClosestDescendantWithLE(int v) {
    return _distToClosestDescendantWithLE[v];
  }
  ...
  virtual size_t build(const std::vector<val_t> *assignment = NULL,
                       bool computeTables = true);

  // compute the _distToClosestDescendantWithLE[] array, based on
  // _BucketErrorQuality[] array.
  int Compute_distToClosestDescendantWithLE(void);

  virtual void noteOrNodeExpansionBeginning(int Var,
                                            std::vector<val_t> &Context,
  ...
  // computes the lookahead heuristic for variable var given a (partial)
  // assignment.
  // note : this fn returns BucketError[var]. all variables in the output fn of
  // bucket[var] should be instantiated by 'assignment'.
  // note : the return value is in the same representation space (log or normal)
  // as the problem.
  // note : the return values should be >= 0.0, regardless of whether normal/log
  // representation.
  ...
  // note : error_fn/avg_error/avg_exact are in the same representation space
  // (normal/log) as the problem itself.
  // note : localerror should always be >=0, regardless of whether normal/log
  // representation, since item_1 is at least as large as item_2 (above).
  // note : the scope of the table (as fn) is the same as the bucket output fn.
  // note : this fn sets _BucketErrorQuality[var] to 0/1+ when it can be
  // determined; sometimes when bucket error table is all 0, we won't compute
  // the table and will set _BucketErrorQuality[var]=0.
  int computeLocalErrorTable(
      int var, bool build_table, bool sample_table_if_not_computed,
      double TableMemoryLimitAsNumElementsLog,
      double &TableSizeLog,  // OUT : bucket error table size, regardless of
                             // whether it is actually computed; this is in log
                             // scale, i.e. sum_log10(var_domain_size).
      double &avgError,  // OUT : avg bucket error; computed when output table
                         // is not too large.
      double &avgExact,  // OUT : avg value over entire bucket output table;
                         // computed when error is computed.
      Function *&error_fn,  // OUT : bucket error fn; computed when output table
                            // is not too large.
      int64 &nEntriesGenerated);  // OUT : number of actual table entires
                                  // computed

  // A version of the above that takes in a partial scope to enumerate,
  // sampling the rest.
  int computeLocalErrorTableSlice(
      ...
      int64 &nEntriesGenerated);  // OUT : number of actual table entires
                                  // computed
  int computeLocalErrorTables(bool build_tables,
  ...
  int localError_H(int var, bool computeTable, double &avgError,
                   Function *&errorFn);
  int localError_H(double &avgError, bool do_printf);

  int deleteLocalErrorFNs(void) {
    for (vector<Function *>::iterator it = _BucketErrorFunctions.begin();
         it != _BucketErrorFunctions.end(); ++it)
...
inline MiniBucketElimLH::MiniBucketElimLH(Problem *p, Pseudotree *pt,
...
inline void MiniBucketElimLH::printExtraStats() const {
  ...
  cout << "Lookahead ratio (OR): " << double(total_lookahead_or) /
                                          total_calls_or << endl;
...
int computeMBOutFnArgsVectorPtrMap(
        ...
        vector<vector<val_t *>> & idxMap // this array can be used in a call to
...
class MiniBucketElimLHErrorNode {
 ...
 public:
  ...
  int _idxWRTparent;  // idx of this node as child of its parent; this is wrt
                      // the childlen list of the parent.
  int _k;             // domain size of this variable.
  int _depth2go;      // depth still to go from this node; as number of edges.
  std::vector<Function *> _RelevantMiniBucketFunctions;  // IF/AF that came from
                                                         // within of the
                                                         // LH-subteee rooted at
                                                         // this node
  std::vector<Function *> _RelevantBucketFunctions;    // OF + AF that came from
                                                       // outside of the
                                                       // LH-subteee rooted at
                                                       // this node
  std::vector<MiniBucketElimLHErrorNode *> _Children;  // children of this node
                                                       // in the bucket tree,
                                                       // that have non-0 bucket
                                                       // error within
                                                       // LH-subteee rooted at
                                                       // this node.
 public:
  int Delete(void) {
    for (vector<MiniBucketElimLHErrorNode *>::iterator it = _Children.begin();
         it != _Children.end(); ++it)
      delete *it;
    _Children.clear();
  ...
  inline double ExactBucketValue(vector<val_t> &assignment) const {
    /*		int i ;
                    vector<Function*>::const_iterator itF_end =
       _RelevantBucketFunctions.end() ;
                    // sum up bucket functions
                    vector<double> funVals ;
                    vector<double> sumVals ;
       sumVals.resize((vector<double>::size_type) _k, ELEM_ONE) ;
                    for (vector<Function*>::const_iterator itF =
       _RelevantBucketFunctions.begin() ; itF != itF_end ; ++itF) {
                            (*itF)->getValues(assignment, _v, funVals) ;
                            for (i = 0 ; i < _k ; ++i)
                                    sumVals[i] OP_TIMESEQ funVals[i] ;
                            }
                    if (_Children.size() > 0) {
                            vector<MiniBucketElimLHErrorNode *>::const_iterator
       itC_end = _Children.end() ;
                            for (i = 0 ; i < _k ; ++i) {
                                    assignment[_v] = i ;
                                    for (vector<MiniBucketElimLHErrorNode
       *>::const_iterator itC = _Children.begin() ; itC != itC_end ; ++itC)
                                            sumVals[i] OP_TIMESEQ
       (*itC)->ExactBucketValue(assignment) ;
                                    }}
    ...
    vector<MiniBucketElimLHErrorNode *>::const_iterator itC_end =
        _Children.end();
    ...
    for (int i = 0; i < _k; ++i) {
      ...
      for (vector<Function *>::const_iterator itF =
               _RelevantBucketFunctions.begin();
           itF != itF_end; ++itF)
        value OP_TIMESEQ(*itF)->getValue(assignment);
      for (vector<MiniBucketElimLHErrorNode *>::const_iterator itC =
               _Children.begin();
           itC != itC_end; ++itC)
  ...
  inline double Error(vector<val_t> &assignment) const {
    if (1 == _depth2go) {
      ...
      if (NULL != eFN) return eFN->getValue(assignment);
    }
    double tableentryMB = ELEM_ONE;
    vector<Function *>::const_iterator itF_end =
        _RelevantMiniBucketFunctions.end();
    for (vector<Function *>::const_iterator itF =
             _RelevantMiniBucketFunctions.begin();
         itF != itF_end; ++itF)
      tableentryMB OP_TIMESEQ(*itF)->getValue(assignment);
    if (OUR_OWN_nInfinity == tableentryMB)
      return 0.0;  // if must be that tableentryB is also -Infinity and error is
                   // 0.
  ...
  // we assume that 'out' is initialized; furthermore, we will subtract
  // heuristic error(s) we compute from current values of 'out'.
  // e.g. typically 'out' is passed in as h/f of 'Parent'.
  inline void Error(int root_var, vector<val_t> &assignment,
    ...
    if (1 == _depth2go) {
      ...
      if (NULL != eFN) {
        ...
        for (k = 0; k < domainsize_root_var; ++k) out[k] OP_DIVIDEEQ funVals[k];
        return;
      }
    }
    vector<double> MBVals;
    MBVals.resize((vector<double>::size_type)domainsize_root_var, ELEM_ONE);
    vector<Function *>::const_iterator itF_end =
        _RelevantMiniBucketFunctions.end();
    for (vector<Function *>::const_iterator itF =
             _RelevantMiniBucketFunctions.begin();
      ...
      for (k = 0; k < domainsize_root_var; ++k) MBVals[k] OP_TIMESEQ funVals[k];
    }
    // compute bucket output for the given assignment; this is the exact bucket
    // value, given its functions and assignment.
    // note here we have two unassigned variables : _v and Child (var). we have
    // to enumerate over one of them.
    for (k = 0; k < domainsize_root_var; ++k) {
      if (OUR_OWN_nInfinity == MBVals[k])
        continue;  // if must be that tableentryB is also -Infinity and error is
                   // 0.
  ...
  MiniBucketElimLHErrorNode(void)
      : _H(NULL), _v(-1), _idxWRTparent(-1), _k(-1), _depth2go(-1) {}
  ~MiniBucketElimLHErrorNode(void) { Delete(); }
};
...
class MiniBucketElimLHError {
 ...
 public:
  bool IsSubtreeVariable(int v) {
    for (vector<MiniBucketElimLHErrorNode *>::iterator itRB =
             _SubtreeNodes.begin();
         itRB != _SubtreeNodes.end(); ++itRB) {
  ...
  bool IsSubtreeMBEfunction(Function &f) {
    for (vector<MiniBucketElimLHErrorNode *>::iterator itRB =
      ...
      for (vector<MiniBucket>::iterator itMB = minibuckets.begin();
           itMB != minibuckets.end(); ++itMB) {
  ...
  inline double Error(vector<val_t> &assignment) const {
    if (0 == _SubtreeNodes.size())
      return 0.0;  // most likely, all the children(buckets) up to depth d have
                   // exactly 1 MB.
    // need to sum errors for each child separately, because MB value for some
    // child could be -infinity and hence if we summed all MB, it would also be
    // -infinity,
    // whereas in reality, some children have 0-error, some non-0-error.
    double e = ELEM_ONE;
    vector<MiniBucketElimLHErrorNode *>::const_iterator itC_end =
        _RootNode._Children.end();
    for (vector<MiniBucketElimLHErrorNode *>::const_iterator itC =
             _RootNode._Children.begin();
         itC != itC_end; ++itC)
      e OP_TIMESEQ(*itC)->Error(assignment);
    return e;
  }
  inline double ErrorPerIndSubproblem(vector<val_t> &assignment,
                                      double current_pruning_gap,
                                      std::vector<double> &subprobH) const {
    if (0 == _SubtreeNodes.size())
      return 0.0;  // most likely, all the children(buckets) up to depth d have
                   // exactly 1 MB.
    // need to sum errors for each child separately, because MB value for some
    ...
    for (vector<MiniBucketElimLHErrorNode *>::const_iterator itC =
      ...
      if (current_pruning_gap <= 0.0)
        break;  // error so far is big enough to cause pruning; no need to
                // improve the error further.
    }
    return e;
  }
  // get heuristic error for each value of the variable '_v'.
  ...
  inline void Error(vector<val_t> &assignment, vector<double> &out) const {
    if (0 == _SubtreeNodes.size())
      return;  // most likely, all the children(buckets) up to depth d have
               // exactly 1 MB.
 ...
 public:
  int Initialize(MiniBucketElimLH &H, int v, int depth) {
    ...
    _depth = depth;
    _RootNode._v = v;
    _RootNode._k = (H.m_problem)->getDomainSize(v);
    _RootNode._depth2go = depth;
    // always check if there is any point in computing error
    if (H._distToClosestDescendantWithLE[v] > depth) return 0;
    // build tree of nodes
    int memory = depth * 5;
    if (memory > H.m_problem->getN())
      memory = H.m_problem->getN();  // assume avg branching factor of 5
    _SubtreeNodes.reserve(
        memory);  // try to allocate some memory to make it faster
                  //		std::deque<MiniBucketElimLHErrorNode *> nodes2process ;
    std::stack<MiniBucketElimLHErrorNode *> nodes2process;  // using stack will
                                                            // turn it into a
                                                            // DFS processing of
                                                            // the tree
    try {
      nodes2process.push(&_RootNode);
    }
    catch (...) {
      return 1;
    }
    ...
    while (nodes2process.size() > 0) {
      MiniBucketElimLHErrorNode *N = nodes2process.top();
      nodes2process.pop();
      ...
      N->_Children.reserve(children.size());  // reserver space for all children
                                              // (some will not be added as
                                              // children) so that reallocation
                                              // is not needed
      int depth2go = N->_depth2go - 1;
      int idxChild = children.size() - 1;
      for (vector<PseudotreeNode *>::const_reverse_iterator
               itC = children.rbegin();
           itC != children.rend(); ++itC, --idxChild) {
        int child = (*itC)->getVar();
#ifndef USE_FULL_LOOKAHEAD_SUBTREE
        if (H._BucketErrorQuality[child] <= 1
                ? H._distToClosestDescendantWithLE[child] > depth2go
                : false)
          continue;  // _BucketErrorQuality <= 1 means it is not proven that
                     // there is substantial bucket error
#endif               // USE_FULL_LOOKAHEAD_SUBTREE
        MiniBucketElimLHErrorNode *n = NULL;
        try {
          n = new MiniBucketElimLHErrorNode;
        }
        catch (...) {
          return 1;
        }
        ...
        n->_H = &H;
        n->_v = child;
        n->_k = (H.m_problem)->getDomainSize(child);
        n->_depth2go = depth2go;
        try {
          _SubtreeNodes.push_back(n);
        }
        catch (...) {
          delete n;
          return 1;
        ...
        // add all MB output functions of [child] bucket that are in [v] bucket
        // to currentrootchild
        MiniBucketElimLHErrorNode *currentrootchild_ =
            (NULL == currentrootchild) ? n : currentrootchild;
        ...
        for (vector<MiniBucket>::iterator itMB = minibuckets.begin();
          ...
          if (vAFlist.end() ==
                  std::find(vAFlist.begin(), vAFlist.end(), fnMB) &&
              vIFlist.end() == std::find(vIFlist.begin(), vIFlist.end(), fnMB))
            continue;
          try {
            (currentrootchild_->_RelevantMiniBucketFunctions).push_back(fnMB);
          }
          catch (...) {
            return 1;
          }
    ...
    for (vector<MiniBucketElimLHErrorNode *>::iterator itRB =
      ...
      // add AF_n^{>d}; i.e. those functions in AF_n that did not come from a
      // subtree bucket
      vector<Function *> &AFlist = H.m_augmented[u];
      for (vector<Function *>::iterator itF = AFlist.begin();
           itF != AFlist.end(); ++itF) {
        Function *fn = *itF;
    ...
    bucket first
                    for (vector<MiniBucketElimLHErrorNode *>::iterator itRB =
    _SubtreeNodes.begin(); itRB!=_SubtreeNodes.end(); ++itRB) {
                            MiniBucketElimLHErrorNode *n = *itRB ;
                            int u = n->_v ;
                            vector<MiniBucket> & minibuckets = H._MiniBuckets[u]
    ;
                            for (vector<MiniBucket>::iterator itMB =
    minibuckets.begin() ; itMB != minibuckets.end() ; ++itMB) {
...
class MiniBucketElimLHheuristicNode {
 ...
 public:
  ...
  int _k;             // domain size of this variable.
  int _depth2go;      // depth still to go from this node; as number of edges.
  std::vector<Function *> _RelevantBucketFunctions;  // OF + AF (in this bucket)
                                                     // that came from outside
                                                     // of the LH-subteee
  std::vector<MiniBucketElimLHheuristicNode *> _Children;  // children of this
                                                           // node in the bucket
                                                           // tree, that have
                                                           // non-0 bucket error
                                                           // within LH-subteee
 ...
 public:
  int Delete(void) {
    for (vector<MiniBucketElimLHheuristicNode *>::iterator it =
             _Children.begin();
         it != _Children.end(); ++it)
      delete *it;
    _Children.clear();
  ...
  inline double ExactBucketValue(vector<val_t> &assignment) const {
    vector<Function *>::const_iterator itF_end = _RelevantBucketFunctions.end();
    vector<MiniBucketElimLHheuristicNode *>::const_iterator itC_end =
        _Children.end();
    double valueMax = ELEM_ZERO;
#ifdef DEBUG
    bool assign_is_none = false;
    ...
    for (int i = 0; i < _k; ++i) {
      ...
      for (vector<Function *>::const_iterator itF =
           ...
           itF != itF_end; ++itF)
        value OP_TIMESEQ(*itF)->getValue(assignment);
      for (vector<MiniBucketElimLHheuristicNode *>::const_iterator itC =
               _Children.begin();
           itC != itC_end; ++itC) {
        value OP_TIMESEQ(*itC)->ExactBucketValue(assignment);
        if (OUR_OWN_nInfinity == value)
          goto done_this_iteration;  // no point in continuing with the
                                     // computation
      }
      valueMax = max(valueMax, value);
    done_this_iteration:
  ...
  MiniBucketElimLHheuristicNode(void)
      : _H(NULL), _v(-1), _idxWRTparent(-1), _k(-1), _depth2go(-1) {}
  ~MiniBucketElimLHheuristicNode(void) { Delete(); }
};
...
class MiniBucketElimLHheuristic {
 ...
 public:
  ...
  std::vector<MiniBucketElimLHErrorNode *> _SubtreeNodes;  // variables/buckets
                                                           // in the (minimal)
                                                           // subtree
  MiniBucketElimLHErrorNode _RootNode;  // node corresponding to _v
  std::vector<Function *> _IntermediateSubtreeFunctions;  // IF/AF (in bucket of
                                                          // _v) that came from
                                                          // outside of the
                                                          // LH-subteee
 public:
  ...
  inline double GetHeuristic(vector<val_t> &assignment) const {
    if (0 == _SubtreeNodes.size())
      return _H->MiniBucketElim::getHeur(
          _v, assignment, NULL);  // most likely, all the children(buckets) up
                                  // to depth d have exactly 1 MB.

    // add up all lambda functions of _v that came from outside the LH subtree;
    // these functions are fully instantiated
    double h = ELEM_ONE;
    vector<Function *>::const_iterator itF_end =
        _IntermediateSubtreeFunctions.end();
    for (vector<Function *>::const_iterator itF =
             _IntermediateSubtreeFunctions.begin();
         itF != itF_end; ++itF) {
      h OP_TIMESEQ(*itF)->getValue(assignment);
      if (OUR_OWN_nInfinity == h)
        return OUR_OWN_nInfinity;  // no point in continuing with the
                                   // computation
    }
 ...
 public:
  int Initialize(MiniBucketElimLH &H, int v, int depth) {
    ...
    // fill in _IntermediateSubtreeFunctions - IF/AF (in bucket of _v) that came
    // from outside of the LH-subteee
    vector<Function *> &vAFlist = H.m_augmented[v];
    for (vector<Function *>::iterator itF = vAFlist.begin();
         itF != vAFlist.end(); ++itF) {
      Function *fn = *itF;
    ...
    vector<Function *> &vIFlist = H.m_intermediate[v];
    for (vector<Function *>::iterator itF = vIFlist.begin();
         itF != vIFlist.end(); ++itF) {
      Function *fn = *itF;
      if (!IsSubtreeMBEfunction(*fn))
        _IntermediateSubtreeFunctions.push_back(fn);
