...
class Main {
 ...
 public:
  ...
  bool setFGLPOptions(int mplp, int mplps);
  bool setJGLPOptions(int jglp, int jglps);
  bool setIboundOptions(int ibound);
  ...
  bool compileHeuristic(int task); //task0 FGLP, task1 JGLP taks2 MBEMM --> calls to build() of heuristic objects
  bool compileHeuristicBeforeBuild();// to be used with compileHeuristic(int);
  bool compileHeuristicAfterBuild(); // to be used with compileHeuristic(int);
  bool runLDS();
  bool finishPreproc();
  bool runSearch(size_t nodeLimit = 0);
  bool outputStats() const;
  int outputStatsToFile() const;
  int64 getNumORnodesExpanded() const { return m_space->stats.numExpOR ; }
