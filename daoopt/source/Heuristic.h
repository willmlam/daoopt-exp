...
public:
	...
	inline Problem *getProblem(void) const { return m_problem ; }
	inline Pseudotree *getPseudotree(void) const { return m_pseudotree ; }
	inline ProgramOptions *getProgramOptions(void) const { return m_options ; }
  ...
  /* OR-node corresponding to the given variable is being expanded. Context is assignment of previously instnatiated variables.
   * This function is called to allow the hueristic to precompute stuff for OR-node expansion.
   * E.g. For a lookahead heuristic, we may want to compute lookahead for all values of Var at this point.
   */
  ...
  virtual size_t build(int task, const std::vector<val_t>* = NULL,
