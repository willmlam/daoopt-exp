...
bool Main::runSLS() {
  ...
  switch (m_options->slsAlgo) {
	case 1: m_slsWrapper.reset(new SLSWrapper()); break;
	case 2: m_slsWrapper.reset(new SLSWrapperHybrid()); break;
	default: m_slsWrapper.reset(new SLSWrapperInc(m_options->slsConvergeRate)); break;
  }
...
bool Main::compileHeuristicBeforeBuild() {
	...
	m_options->jglpi = m_pseudotree->getWidth(); //min(m_options->jglpi, m_pseudotree->getWidthCond());
	size_t sz = 0;
	
	// do not limit ibound in before running classifier. it should be remained as 0.
...
// DaooptInterface::compileFGLP(), DaooptInterface::compileJGLP(), DaooptInterface::compileMbeAndFinishPreprocessing()
// ans such methods calls, _daoopt.compileFGLP() etc will be triggerd inside daoopt trhead function
bool Main::compileHeuristic(int task) {
	...
	if (m_options->nosearch) {
		...
		if (!mbFromFile)
			sz = m_heuristic->build(task, &m_search->getAssignment(), true); // true =  actually compute heuristic

		if (task == 2) {
			high_resolution_clock::time_point cur_time = high_resolution_clock::now();
			double time_passed = duration_cast<duration<double>>(cur_time - _time_pre).count();
...
// orignial method for compiling Heuristic that calls Build method for the Heuristic obejct
...
bool Main::exportProblemStats(int& fN, int& fF, int& fK, int& fS, int& fW, int& fH) {
	fN = m_problem->getN();
	fF = m_problem->getC();
	fK = m_problem->getK();
	fS = m_problem->getR();
