...
//  vector<Function*> newFunctions(factors.size()); // to hold replacement, reparameterized functions
  ...
  Function *constFun = new FunctionBayes(newFunctions.back()->getId()+1,m_problem,set<int>(),table1,1);
...
size_t FGLPMBEHybrid::computeMBEMemory(int ibound) {
    ...
    vector<vector<Function*> > augmented(m_problem->getN(),vector<Function*>());

    for (vector<int>::const_iterator itV=elimOrder.begin(); itV!=elimOrder.end(); ++itV) {
