...
int ARE::Function::ConvertTableToLogScale(void)
{
	if (_ConstValue >= 0.0) {
...
int ARE::Function::AllocateInMemoryAsSingleTableBlock(void)
{
	// check that nothing is allocated yet
	if (NULL != _FTBsInMemory) 
		// there are already blocks allocated/in-memory.
		return 1 ;
...
*/
	...
	if (NULL != _Table) {
		if (0 != _Table->IDX()) {
			// table block that does not belong to this fn already exist; we assumed that table is unallocated; fail.
			return 1 ;
			}
		if (_Table->Size() != _TableSize) {
			delete _Table ;
			_Table = NULL ;
			}
		}
