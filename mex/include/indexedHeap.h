...
/* Indexed Heap : a "reversible" heap from key (double) to unique values (uint,
 *0..N-1; some can be missing)
 *   for which the value can also be used to look up (access) or remove an entry
 *
 * Mainly this data structure is used in factor- or edge- priority scheduling;
 *the factor or edge index
 *   is used as the unique value and its priority is the key.  This gives a
...
class indexedHeap : virtual public mxObject {
 ...
 private:
  ...
  mex::vector<uint32_t> _rev;  // reverse lookup from value to position in the
                               // heap
 ...
 public:
  ...
  indexedHeap(InputPIterator pBegin, InputPIterator pEnd, InputIIterator iBegin)
      : _p(), _id(), _rev() {
  ...
  // Insert or change the priority of an element with (unique) ID "R" and
  // priority "P"
  void insert(double P, uint32_t R) {
    if (_rev.size() <= R)
      _rev.resize(R + 1, 0);  // make sure we have enough space for ID
    size_t i;
    if (_rev[R] != 0) {  // if index "R" already exists
      i = _rev[R];
      _p[i - 1] = P;  //   just change its priority
      maxHeapify(i);  //   and push down heap (so, heapified below)
    } else {          // otherwise, add it at the end
      _p.push_back(P);
      _id.push_back(R);
      _rev[R] = size();  // _rev index is 1...N
      i = size();
    }
  ...
  void test() {
    ...
    std::cout << "M: " << M_ << " - " << mxGetField(M_, 0, "p") << " @ "
              << mxGetPr(mxGetField(M_, 0, "p")) << "\n";
    std::cout << "m: " << m << " - " << mxGetField(m, 0, "p") << " @ "
              << mxGetPr(mxGetField(m, 0, "p")) << "\n";
  ...
  virtual mxArray* mxGetRef() {
    if (!mxAvail())
      return mxGet();  // if this will be new in matlab, create it
    else
      return mxCreateSharedDataCopy(
          mxGet());  // otherwise create a shared version
  }
 ...
 private:
  ...
  void maxHeapify(uint32_t i) {
    for (;;) {
      uint32_t left = 2 * i, right = 2 * i + 1,
               largest = i;  // find largest of i, L(i), R(i)
      if (left <= _p.size() && _p[left - 1] > _p[largest - 1]) largest = left;
      if (right <= _p.size() && _p[right - 1] > _p[largest - 1])
        largest = right;
