...
public:
  ...
  void run() {
    ...
    if (iter>print*nFactors()) { 
			...
			vector<uint32_t> config = tmp.maxSequential(order(mex::graphModel::OrderMethod::Random), false);
			//vector<uint32_t> config = maxSequentialFast(order(mex::graphModel::OrderMethod::Random));
			//std::cout<<logP(config)<<" ("<<logP(_best)<<") : "; for (size_t v=0;v<config.size();++v) std::cout<<config[v]<<" "; std::cout<<"\n";
