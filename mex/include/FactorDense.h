...
class Factor : public virtual mxObject {
 public:
  typedef double value;                                    // factor "type" (not templated)
  ...
  // MEX Class Wrapper Functions //////////////////////////////////////////////////////////
   ...
   mxArray*    mxGet();                      // get a pointer to the matlab object wrapper (creating if req'd)
  ...
  void marginalInto(VarSet const& target, Factor& F) const;  // marginal "in-place" into another function
  // !!! TODO: max or min marginal Into? lseMarginalInto?

  // !!! TODO: lseMarginal?
  // !!! TODO: power marginals? lsePowerMarginals?
...
protected:
  ...
  #ifdef MEX                  // helper functions: infinite & NaN values (different for Matlab!)
  ...
  /////////////////////////////////////////////////////////////////////////////
  // Templated private members for implementation of various operations      //
  /////////////////////////////////////////////////////////////////////////////
  template <class xform>
  inline void transform(xform x) { std::transform(t_.begin(),t_.end(),t_.begin(),x); }
...
inline Factor::Factor(const value s) : v_(),   t_(1,s)  {             // scalar constructor
	FACTOR_ADD_MEMORY( t_.capacity()*sizeof(double) );
}
...
inline const Factor::value*  Factor::table()     const { return &t_[0];   };  // table of factor values
inline size_t                Factor::nrStates()  const { return t_.size(); }  // get table size
inline size_t                Factor::numel()     const { return t_.size(); }; // name difference (!!)

// Boolean checks on factor properties //////////////////////////////////////////////////
inline bool Factor::isempty()  const { return t_.empty(); };             // empty factor
inline bool Factor::isnan()    const { bool b=false; for (size_t i=0;i<nrStates();i++) b|=isnan(t_[i]);    return b; };
inline bool Factor::isfinite() const { bool b=true;  for (size_t i=0;i<nrStates();i++) b&=isfinite(t_[i]); return b; };
inline bool Factor::isscalar() const { return numel()==1; };

inline void Factor::cleanInf() { for (size_t i=0;i<nrStates();i++) if (!isfinite(t_[i])) t_[i]=0.0; };

// Direct value accessor ////////////////////////////////////////////////////////////////
inline Factor::value  Factor::operator[] (vsize v) const { return t_[v]; };      // access table elements
...
// Filling
inline Factor& Factor::fill(value v) { std::fill(t_.begin(),t_.end(),v); return *this; }  // fill with constant
inline Factor& Factor::randomize()   { std::generate(t_.begin(),t_.end(),mex::randu); return *this; }
...
inline Factor::value  Factor::sum()                     const { return std::accumulate(t_.begin(),t_.end(),0.0,std::plus<value>()); }

inline Factor         Factor::max(VarSet const& sumOut) const { VarSet t=v_ - sumOut; return maxmarginal(t); }
inline Factor::value  Factor::max()                     const { return std::accumulate(t_.begin(),t_.end(),-infty(),myMax()); }

inline Factor         Factor::min(VarSet const& sumOut) const { VarSet t= v_ - sumOut; return minmarginal(t); }
inline Factor::value  Factor::min()                     const { return std::accumulate(t_.begin(),t_.end(),infty(),myMin()); }

inline size_t Factor::argmax() const { return std::distance(t_.begin(), std::max_element(t_.begin(),t_.end())); }
inline size_t Factor::argmin() const { return std::distance(t_.begin(), std::min_element(t_.begin(),t_.end())); }
