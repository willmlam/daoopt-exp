...
#define dLocal_EXP	dLocal.data()
#elif defined (LINUX)
#define dLocal_EXP	&dLocal[0]
#endif 
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  VarSet(const VarSet& vs)            : v_(vs.v_), dLocal(vs.d_,vs.d_+vs.size()) { d_=dLocal_EXP; }
  VarSet(const Var& v)                : v_(1,v.label()), dLocal(1,v.states())    { d_=dLocal_EXP; }  
  ...
  VarSet(iter B,iter E, size_t size_hint=0) : v_(), dLocal() {
    v_.reserve(size_hint); dLocal.reserve(size_hint); d_=dLocal_EXP;
    for (;B!=E;++B) *this|=*B; 
  }
  // Destructor /////////////////////////
  ~VarSet(void) { }
  ...
  VarSet& operator= (const VarSet& B) {
    v_=B.v_; dLocal=vector<vsize>(B.d_,B.d_+B.size()); d_=dLocal_EXP;
//#ifdef MEX
...
//#endif
  ...
  void clear(void) { v_.clear() ; dLocal.clear() ; d_ = NULL ; }
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  const Var      last()         const { assert(size()!=0); return Var(v_[size()-1],d_[size()-1]); }
  const Var      first()        const { assert(size()!=0); return Var(v_[0],d_[0]); }
...
// Operators on single-element arguments
  ...
  VarSet& operator-=( const VarSet& B ) {
    ...
    while (vi!=v_.end())   { *dk++=*di++; *vk++=*vi++; }
    size_t n=std::distance(dest.v_.begin(),vk); dest.v_.resize(n); dest.dLocal.resize(n); d_=dLocal_EXP;
    return *this;
  }
...
void mxSet(mxArray* M, const vsize* dnew) { 
  v_.mxSet(M); 
  if (dnew) d_=dnew; else { dLocal.resize(size()); d_=dLocal_EXP; std::fill(dLocal.begin(),dLocal.end(),0); }
}
mxArray* mxGet() { return v_.mxGet(); }
void mxRelease() { v_.mxRelease(); if (!isLocal()) { dLocal=vector<vsize>(d_,d_+size()); d_=dLocal_EXP; } }
void mxDestroy() { v_.mxDestroy(); if (!isLocal()) { dLocal=vector<vsize>(d_,d_+size()); d_=dLocal_EXP; } }
...
protected:
  ...
  bool isLocal() { return (d_==dLocal_EXP); }    // check for local storage of dimensions
  explicit VarSet(const size_t Nv) : v_(Nv), dLocal(Nv) { d_=dLocal_EXP; }  // "reserved memory" constructor
...
inline void ind2sub(const VarSet& vs, size_t i, MapType& val) {
  ...
  for (size_t v=0; v<vs.size(); ++v) {
		if (vs[v].states()<=1) { val[vs[v]]=0; continue; }
...
inline vector<uint32_t> ind2sub(const VarSet& vs, size_t i) {
  ...
  for (size_t v=0; v<vs.size(); ++v) {
		if (vs[v].states()<=1) { val[v]=0; continue; }
