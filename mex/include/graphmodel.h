...
public:
  ...
  findex largest(const flist& fl);
	...
	void condition(const VarSet& cond, const MapType& val) {
		for (size_t f=0;f<_factors.size();++f) {
			VarSet overlap = _factors[f].vars() & cond;
			if (overlap.size() > 0) {
				//erase(_vAdj,f,factor(f).vars());
				//factor(f) = factor(f).condition(overlap, sub2ind(overlap,val));
				//insert(_vAdj,f,factor(f).vars());
				Factor F = _factors[f].condition(overlap, sub2ind(overlap,val));
				removeFactor(f);
				// if empty factor, do something else?
				addFactor(F);
			}
		}
		for (VarSet::const_iterator v=cond.begin();v!=cond.end();++v) addFactor(Factor::delta(*v,val[*v]));
	}
  ...
  /// simple optimum selection routines !!! TODO: return VarConfig type?
  ...
  vector<uint32_t> maxSequential( const VarOrder& ord, bool isLog=true );
  vector<uint32_t> maxSequentialFast( const VarOrder& ord, bool isLog=true );

  // logP( config ): compute the log-probability of a particular (complete) configuration
	template <class MapType>
  inline double logP( const MapType& config ) {   
    double val = 0.0;
