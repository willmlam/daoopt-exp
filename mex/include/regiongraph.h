...
public:
  ...
  vector<findex> sort(const flist& fs) const {
  ...
  double       count(findex r)    const { return _count[r]; }
  ...
  void calculateCount() {
    ...
    for (std::map<VarSet,findex>::reverse_iterator it=_regionIndex.rbegin();it!=_regionIndex.rend();++it) {
      ...
      for (flist::const_iterator p=ancestors.begin(); p!=ancestors.end(); ++p) if (r!=*p) _count[r]-=_count[*p];
    }
  }
  ...
  void calculateCount(findex r) {
    ...
    for (flist::const_iterator p=ancestors.begin(); p!=ancestors.end(); ++p) if (*p!=r) _count[r]-=_count[*p];
    vector<findex> descend = sort(containedBy(_regions[r])); // Now update all of r's descendants:
    for (vector<findex>::reverse_iterator c=descend.rbegin(); c!=descend.rend(); ++c) {
      _count[*c]=1.0;
      flist ancestors = contains(_regions[*c]);   //   find their ancestors and recompute
      for (flist::const_iterator p=ancestors.begin(); p!=ancestors.end(); ++p) if (*p!=*c) _count[*c]-=_count[*p];
    }
  }

  void cleanUp(void) {
    for (size_t r=0;r<nregions();++r) if (_count[r] == 0.0) {
      _regionIndex.erase( _regions[r] );
      graphModel gm; gm.erase(_vAdj, r, _regions[r]);
      _regions[r]=VarSet();
    }
  }

	// !!! TODO:DEBUG : run through all regions and check maximal settings, fix if required	
	void fixMaximal(void) {
		size_t added(0), removed(0), orig(_maximalRegions.size());
		for (size_t r=0;r<nregions();++r) {
			bool isMaximalSet = _maximalRegions.contains(r);
			bool isMaximal = (contains(_regions[r]).size()==1);
			if (isMaximal && !isMaximalSet) { added++; _maximalRegions+=r; }
			if (!isMaximal && isMaximalSet) { removed++; _maximalRegions-=r; }
		}
		std::cout<<"Initially "<<orig<<" maximal regions; removed "<<removed<<" and added "<<added<<" = "<<_maximalRegions.size()<<"\n";
	}


  // Insert a new region (but not any extra regions besides that one)
  bool insertRegion(const VarSet& vs) {
    if (vs.size()==0) return false;
    if (_regionIndex.count(vs)) return false;          // already in our region graph?
    if (_vAdj.size()>vs.rbegin()->label() && contains(vs).size()==1) return false; // !!! unique parent = no cycles
    ...
    if (ms.size()==0) {                        // if it's a new maximal region
      ms = _maximalRegions & containedBy(vs);  //   find out if it subsumes any others
      _maximalRegions -= ms;                   //   and if so push them down
      _maximalRegions += idx;
...
*/
  ...
  void dump() {
    ...
    for (std::map<VarSet,findex>::reverse_iterator it=_regionIndex.rbegin();it!=_regionIndex.rend();++it) {
      findex r = it->second;
			if (_maximalRegions.contains(r)) std::cout<<"*"; else std::cout<<" ";
  ...
  flist addRegion(const VarSet& vs, bool updateCount=true) {
    if (vs.size()==0) return flist();
    flist nbrs = intersects(vs);      // find all the regions we could intersect with
    findex first = _regions.size();   //   (to check which regions we add)
    if (insertRegion(vs)==false) return flist();  // insert vs itself, then any intersections with existing regions
    ...
    findex last = _regions.size();
    flist added; for (findex i=first; i!=last; i=1+i) added+=i;
    return added;
  }
