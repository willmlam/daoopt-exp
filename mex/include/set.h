...
class set : public virtual mxObject, protected mex::vector<T> {
 public:
  ...
  set<T>& insert(const T& v) { return *this+=v; }
  set<T>& erase(const T& v)  { return *this-=v; }
  bool operator<< (const set<T>& S) const { return std::includes( S.begin(),S.end(), this->begin(),this->end() ); }
  bool operator>> (const set<T>& S) const { return std::includes( this->begin(),this->end(), S.begin(),S.end() ); }
  bool intersects(const set<T>& S) const { return (*this & S).size() > 0; }
  bool contains(const T& v) const { return std::binary_search( this->begin(),this->end(), v ); }
