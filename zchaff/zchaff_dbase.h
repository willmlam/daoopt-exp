...
class CDatabase {
  ...
  protected:
    ...
    // note: it will return 0 or 1 or other, here "other" may not equal ZCHAFF_UNKNOWN
  ...
  public:
    ...
    inline unsigned num_clauses(void) {
      return (unsigned)(_clauses.size() - _unused_clause_idx.size());
    }

    inline unsigned num_literals(void) {
      return (unsigned)(_stats.num_added_literals - _stats.num_deleted_literals);
    }
