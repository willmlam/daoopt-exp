...
namespace zchaff {
	...
	unsigned CDatabase::estimate_mem_usage(void) {
		unsigned mem_lit_pool = sizeof(CLitPoolElement) * (lit_pool_size() +
			lit_pool_free_space());
		unsigned mem_vars = (unsigned)(sizeof(CVariable) * variables()->capacity());
		unsigned mem_cls = (unsigned)(sizeof(CClause) * clauses()->capacity());
		unsigned mem_cls_queue = (unsigned)(sizeof(int) * _unused_clause_idx.size());
	...
	unsigned CDatabase::mem_usage(void) {
		...
		int mem_vars = (int)(sizeof(CVariable) * variables()->capacity());
		int mem_cls = (int)(sizeof(CClause) * clauses()->capacity());
		int mem_cls_queue = (int)(sizeof(int) * _unused_clause_idx.size());
		int mem_watched = 0, mem_lit_clauses = 0;
		for (unsigned i = 0, sz = (unsigned)variables()->size(); i < sz ;  ++i) {
			CVariable & v = variable(i);
			mem_watched        += (int)(v.watched(0).capacity() + v.watched(1).capacity());
#ifdef KEEP_LIT_CLAUSES
			mem_lit_clauses += (int)(v.lit_clause(0).capacity() + v.lit_clause(1).capacity());
#endif
		}
	...
	void CDatabase::mark_clause_deleted(CClause & cl) {
		...
		_unused_clause_idx.insert((ClauseIdx)(&cl - &(*clauses()->begin())));
	}
	...
	inline int CDatabase::lit_pool_size(void) {
		return (int)(_lit_pool_finish - _lit_pool_start);
	}

	inline int CDatabase::lit_pool_free_space(void) {
		return (int)(_lit_pool_end_storage - _lit_pool_finish);
	}
	...
	void CDatabase::compact_lit_pool(void) {
		...
		for (i = 1, sz = (unsigned)variables()->size(); i < sz;  ++i) {
	...
	bool CDatabase::enlarge_lit_pool(void) {
		...
		int old_size = (int)(_lit_pool_end_storage - _lit_pool_start);
		...
		int displacement = (int)(_lit_pool_start - old_start);
		for (i = 0; i < clauses()->size(); ++i) {
			if (clause(i).status() != DELETED_CL)
				clause(i).first_lit() += displacement;
		}
		for (i = 0, sz = (unsigned)variables()->size(); i < sz ;  ++i) {
			CVariable & v = variable(i);
			for (int j = 0; j < 2 ; ++j) {
				int k, sz1;
				vector<CLitPoolElement *> & watched = v.watched(j);
				for (k = 0, sz1 = (int)watched.size(); k < sz1 ; ++k) {
	...
	ClauseIdx CDatabase::get_free_clause_idx(void) {
		ClauseIdx new_cl;
		new_cl = (ClauseIdx)_clauses.size();
