...
#define _CLS_STATUS_
	...
	class CVariable {
	protected:
		unsigned _value             : 2;  // it can take 3 values, 0, 1 and ZCHAFF_UNKNOWN
		bool _marked                : 1;  // used in conflict analysis.
		unsigned _new_cl_phase      : 2;  // it can take 3 value
		// 0: pos phase, 1: neg phase, ZCHAFF_UNKNOWN : not in new clause;
	...
	public:
		...
		void init(void) {
			_value = ZCHAFF_UNKNOWN;
			_antecedent = NULL_CLAUSE;
			_marked = false;
			_dlevel = -1;
			_assgn_stack_pos = -1;
			_new_cl_phase = ZCHAFF_UNKNOWN;
