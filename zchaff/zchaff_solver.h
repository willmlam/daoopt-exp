...
#define _SAT_STATUS_
	...
	protected:
		...
		vector<vector<int> >	_netvar2satvar;
		vector<pair<int,int> >	_satvar2netvar;
	...
	protected:
		...
		void learn_conflict(); // added by Radu (one level clause learning)
		...
		ClauseIdx finish_add_conf_clause_one_step(int gflag);
		int conflict_analysis_firstUIP(void);
		int conflict_analysis_firstUIP_one_step(void);
	...
	public:
		...
		// by Radu
//		void init(CProblem *prob);
//		bool init(CProblem *prob, std::vector<CFunction*>& funs);
		ClauseIdx add_nogood_clause(int n, int *vars, int *vals, int gid = 0);
		ClauseIdx add_atmostone_clause(int var, int val1, int val2, int gid = 0);
		ClauseIdx add_atleastone_clause(int var, int values, int gid = 0);
		void set_var_maps(vector<vector<int> >& var2sat, vector<pair<int, int> >& sat2var);
		void update_domains(vector<vector<bool> > &domains, list<pair<int,int> > &changes);
		...
		inline char * version(void) {
			return (char*)__ZCHAFF_VERSION__;
		}

		double elapsed_cpu_time(void);
		double cpu_run_time(void) ;
		...
		void reset(void);
		void force_reset(void); // by Radu
		int solve(void);

		int propagate(int var, int val);
		int propagate(list<pair<int,int> > &assignments);
		void release_assignments();
		void assert_assignments(std::list<std::pair<int,int> > &assignments);
		...
		void dump_variable_domains(ostream & os = cout);
