...
namespace zchaff {
	...
	void CSolver::re_init_stats(void) {
		...
		_stats.start_cpu_time               = 0; //get_cpu_time();
	...
	ClauseIdx CSolver::add_nogood_clause(int n, int *vars, int *vals, int gid)
	{
		set<int> clause_vars;
		set<int> clause_lits;
		int var_idx;
		int sign = 1;	// negative literals.
		ClauseIdx cid;

		// Create the clause.
		for (int i = 0; i < n; ++i)
		{
			var_idx = _netvar2satvar[vars[i]][vals[i]];
		...
		if (clause_vars.size() != 0 && (clause_vars.size() == clause_lits.size())) { //yeah, can add this clause
			...
			for (set<int>::iterator itr = clause_lits.begin();
				...
				temp.push_back (*itr);
			cid = add_orig_clause(& temp.begin()[0], (int)temp.size(), gid);
		}
	...
	ClauseIdx CSolver::add_atmostone_clause(int var, int val1, int val2, int gid)
	{
		set<int> clause_vars;
		set<int> clause_lits;
		int var_idx;
		...
		// First value in clause.
		var_idx = _netvar2satvar[var][val1];
		clause_vars.insert(var_idx);
		clause_lits.insert( (var_idx << 1) + sign);
		// Second value in clause.
		var_idx = _netvar2satvar[var][val2];
		clause_vars.insert(var_idx);
		clause_lits.insert( (var_idx << 1) + sign);
	...
	ClauseIdx CSolver::add_atleastone_clause(int var, int values, int gid)
	{
		set<int> clause_vars;
		set<int> clause_lits;
		int var_idx;
		int sign = 0;	// positive literals.
		ClauseIdx cid;

		// Create the clause.
		for (int k = 0; k < values; ++k) {
			var_idx = _netvar2satvar[var][k];
			clause_vars.insert(var_idx);
			clause_lits.insert( (var_idx << 1) + sign);
		}
...
/*
	// Init the database from a list of functions and their corresponding vars
	bool CSolver::init(CProblem *prob, std::vector<CFunction*>& funs) {
		// Safety checks
		if (funs.empty()) {
			return false; // ignore an empty subproblem instance
		}

		// Identify the variables in the list of functions
		std::set<int> distinctVars;
		std::vector<CFunction*>::iterator fi = funs.begin();
		for (; fi != funs.end(); ++fi) {
			CFunction *fn = (*fi);
			assert(fn->type() == FUN_CPT);
			std::vector<int>& sc = fn->scope();

			distinctVars.insert(sc.begin(), sc.end());
		}

		// Check if the current subproblem contains any variables
		if (distinctVars.empty()) {
			return false; // ignore an empty subproblem instance
		}

		int n = prob->get_n(), vid = 1, nvars = 0;
		std::set<int>::iterator si = distinctVars.begin();
		for (; si != distinctVars.end(); ++si) {
			int var = (*si);
			int dom = prob->getStaticDomainSize(var);
			nvars += dom;
		}

		// Create variables from the belief network. For each variable
		// value assignment (e.g. X=x) create a sat variable X_x.
		_netvar2satvar.resize(n);
		_satvar2netvar.resize(nvars+1,make_pair(-1,-1));
		set_variable_number(nvars);
		for (si = distinctVars.begin(); si != distinctVars.end(); ++si)	{
			int var = (*si);
			int dom = prob->getStaticDomainSize(var);
			_netvar2satvar[var].resize(dom, 0);
			for (int k = 0; k < dom; ++k)	{
				_netvar2satvar[var][k] = vid;
				_satvar2netvar[vid].first = var;
				_satvar2netvar[vid].second = k;
				++vid;
			}
		}
		// From clauses from the 0 probability tuples (conflict clauses).
		int num_cl = 0;
		for (fi = funs.begin(); fi != funs.end(); ++fi) {
			CFunction *fn = (*fi);
			if (FUN_CPT != fn->type()) {
				continue;
			}
			...
			while (true) {
				for (i = argc-1; i >= 0; --i) {
					int arg = argv[i];
					int last = prob->getStaticDomainSize(arg) - 1;
				...
				for (int a = 0; a < argc; ++a) {
					prob->setValue(argv[a], vals[a]);
				}

				// Add a nogood clause.
				double cval;
				fn->getCurrentValue(cval);
				if (0.0 == cval) {
					add_nogood_clause(argc, argv, vals);
					++num_cl;
				}
			}
		...
		for (si = distinctVars.begin(); si != distinctVars.end(); ++si) {
			...
			for (int v1 = 0; v1 < dom-1; ++v1) {
				for (int v2 = v1+1; v2 < dom; ++v2) {
					add_atmostone_clause(var, v1, v2);
					++num_cl;
				}
			}
		}
		...
		for (si = distinctVars.begin(); si != distinctVars.end(); ++si) {
			int var = (*si);
			int dom = prob->getStaticDomainSize(var);
			add_atleastone_clause(var, dom);
			++num_cl;
		}

		// Add unit clauses for evidence variables
		for (si = distinctVars.begin(); si != distinctVars.end(); ++si) {
			int var = (*si);
			if (prob->isEvidence(var)) {
				int val = prob->getValue(var);
				int vid = _netvar2satvar[var][val];
				// Add the implication 'X=x is true'.
				queue_implication(vid+vid, NULL_CLAUSE);
			}
		}

		cout << "\nc number of clauses: " << num_cl;
		cout << "\nc number of variables: " << nvars;
		cout << endl;
	...
	// Init the database from a belief network with 'n' variables.
	void CSolver::init(CProblem *prob)
	{
		int i, k, m, nvars, vid = 1;
		double cval;
		int n = prob->get_n();
		std::vector<int>& domains = prob->getDomains();
		std::vector<CFunction*> &cpts = prob->functions();
		for (i = 0, nvars = 0; i < n; ++i) {
			nvars += domains[i];
		}
		...
		for (i = 0; i < n; ++i)	{
			m = domains[i];
			_netvar2satvar[i].resize(m, 0);
			for (k = 0; k < m; ++k)	{
				_netvar2satvar[i][k] = vid;
				_satvar2netvar[vid].first = i;
				_satvar2netvar[vid].second = k;
				++vid;
			}
		}
		...
		// From clauses from the 0 probability tuples (conflict clauses).
		int num_cl = 0;
		std::vector<CFunction*>::iterator fi = cpts.begin();
		for (; fi != cpts.end(); ++fi) {
			CFunction *fn = (*fi);
			if (FUN_CPT != fn->type()) {
				continue;
		...
		for (i = 0; i < n; ++i) {
			k = domains[i];
			for (int v1 = 0; v1 < k-1; ++v1) {
				for (int v2 = v1+1; v2 < k; ++v2) {
					add_atmostone_clause(i, v1, v2);
					++num_cl;
				}
			}
		}
		...
		for (i = 0; i < n; ++i) {
			k = domains[i];
			add_atleastone_clause(i, k);
			++num_cl;
		}

		// Add unit clauses for evidence variables
		for (i = 0; i < n; ++i) {
			if (prob->isEvidence(i)) {
				int val = prob->getValue(i);
				int vid = _netvar2satvar[i][val];
				// Add the implication 'X=x is true'.
				queue_implication(vid+vid, NULL_CLAUSE);
			}
		}
...
*/
	...
	CSolver::~CSolver(void) {
		...
		for (size_t i = 0; i < _netvar2satvar.size(); ++i)
			_netvar2satvar[i].clear();
		_netvar2satvar.clear();
		_satvar2netvar.clear();
	}
	...
	void CSolver::set_verbosity(int v) {
		_params.verbosity = v;
	}
	...
	double CSolver::cpu_run_time(void) {
	...
	void CSolver::init_solve(void) {
		...
		for (unsigned i = 0, sz = (unsigned)variables()->size(); i < sz; ++i) {
	...
	void CSolver::set_var_value(int v, int value, ClauseIdx ante, int dl) {
		...
		assert(var.value() == ZCHAFF_UNKNOWN);
		assert(dl == dlevel());

		var.set_dlevel(dl);
		var.set_value(value);
		var.antecedent() = ante;
		var.assgn_stack_pos() = (int)(_assignment_stack[dl]->size());
	...
	void CSolver::dump_variable_domains(ostream & os) {
		os << "Current domains:  " << endl;
		//assert(dl <= _assignment_stack.size());

		int n = (int)_netvar2satvar.size();
		vector<vector<bool> > domains;
		domains.resize(n);

		// Loop over all net variables.
		domains.resize(n);
		for (int i = 0; i < n; ++i)
		{
			vector<int>& satvars = _netvar2satvar[i];
			domains[i].resize(satvars.size(), true);
			for (size_t j = 0; j < satvars.size(); ++j)
			{
				int var_idx = satvars[j];
				switch (variable(var_idx).value())
				{
				case 0:		// X=x is pruned from domain
					domains[i][j] = false;
					break;
				case 1:		// X=x is in domain
					break;
				default:	// unassigned
					break;
				}
		...
		for (int i = 0; i < n; ++i)
		{
			os << "x_" << i << " (" << (int)domains[i].size() << ") -- {";
			for (size_t j = 0; j < domains[i].size(); ++j)
				if (domains[i][j]) cout << (int)j << " ";
			os << "}" << endl;
		}
	...
	// set variable maps
	void CSolver::set_var_maps(vector<vector<int> >& var2sat, vector<pair<int, int> >& sat2var) {
		_netvar2satvar = var2sat;
		_satvar2netvar = sat2var;
	}

	// Update the domains from the master problem based on the assignment stack of the sat problem
	void CSolver::update_domains(vector<vector<bool> > &domains, list<pair<int,int> > &changes) {
		int sign, vid, i = dlevel();
		assert(i <= (int)_assignment_stack.size());

		for (size_t j = 0; j < (*_assignment_stack[i]).size(); ++j) {
			sign = ((*_assignment_stack[i])[j]&0x1) ? 1 : 0;
			vid = ((*_assignment_stack[i])[j] >> 1);
			if (sign) { // negative literal, i.e. X != x
				pair<int,int> &p = _satvar2netvar[vid];
				assert((p.first >= 0) && (p.second >= 0));

				// Remove value 'val' from 'var' domain.
				domains[p.first][p.second] = false;
				changes.push_front(p); // record changes for context restoration.
			}
		}
	}
	...
	void CSolver::delete_unrelevant_clauses(void) {
		...
		for (vector<CClause>::iterator itr = clauses()->begin();
			...
			for (vector<CVariable>::iterator itr = variables()->begin();
				...
				for (unsigned i = 1, sz = (unsigned)variables()->size(); i < sz; ++i) {
					if (variable(i).dlevel() != 0) {
						variable(i).score(0) = variable(i).lits_count(0);
						variable(i).score(1) = variable(i).lits_count(1);
						if (variable(i).lits_count(0) == 0 && variable(i).value() == ZCHAFF_UNKNOWN) {
							queue_implication(i * 2 + 1, NULL_CLAUSE);
						}
						else if (variable(i).lits_count(1) == 0 &&
							variable(i).value() == ZCHAFF_UNKNOWN) {
	...
	//			if (variable(s_var >> 1).value() == ZCHAFF_UNKNOWN) {
	...
	//					break;
	//				}
	//				else if (variable(var_idx).value() == ZCHAFF_UNKNOWN) {
	...
	//		if (var.value() == ZCHAFF_UNKNOWN && var.is_branchable()) {
	...
	//			while (skip > 0) {
	//				++index;
	//				if (_ordered_vars[index].first->value() == ZCHAFF_UNKNOWN &&
	//					_ordered_vars[index].first->is_branchable())
	//					--skip;
	//			}
	//			CVariable * ptr = _ordered_vars[index].first;
	//			assert(ptr->value() == ZCHAFF_UNKNOWN && ptr->is_branchable());
	//			int sign = 0;
	...
	int CSolver::preprocess2(void) {
		assert(dlevel() == 0);

		// 1. Unit clauses
		for (unsigned i = 0, sz = (unsigned)clauses()->size(); i < sz; ++i) {
			if (clause(i).status() != DELETED_CL &&
				clause(i).num_lits() == 1 &&
				variable(clause(i).literal(0).var_index()).value() == ZCHAFF_UNKNOWN)
				queue_implication(clause(i).literal(0).s_var(), i);
		}

		// Unit propagation.
		if (deduce() == CONFLICT) {
			cout << "c CONFLICT during preprocess " <<endl;
			return CONFLICT;
		}

		if (_params.verbosity > 1) {
			cout << "c " << (int)_assignment_stack[0]->size()
				<< " vars set during preprocess";
		}
		return NO_CONFLICT;
	}
	...
	int CSolver::preprocess(void) {
		...
		if (_params.verbosity > 1 && un_used.size() > 0) {
			cout << "c " << (int)un_used.size() << " variables are defined but not used " << endl;
			if (_params.verbosity > 2) {
		...
		if (_params.verbosity > 1 && uni_phased.size() > 0) {
			cout << "c " << (int)uni_phased.size() << " variables only appear in one phase." <<endl;
			if (_params.verbosity > 2) {
		...
		if (deduce() == CONFLICT) {
			...
			for (unsigned i = 1; i < variables()->size(); ++i) {
				if (variable(i).value() != ZCHAFF_UNKNOWN) {
		...
		if (_params.verbosity > 1) {
			cout << "c " << (int)_assignment_stack[0]->size()
				<< " vars set during preprocess" << endl;
		}
		return NO_CONFLICT;
	}
	...
	//		if (variable(vid).value() == ZCHAFF_UNKNOWN)
	//			--num_free_variables();
	//	}
	//}
	...
	// Release the assignments at current dlevel.
	void CSolver::release_assignments() {
		int i = dlevel();
		vector<int> & assignments = *_assignment_stack[i];
		for (int j = (int)assignments.size() - 1 ; j >= 0; --j)
			unset_var_value(assignments[j]>>1);
	...
	// Propagate a variable-value from the master problem.
	int CSolver::propagate(int var, int val) {
		int vid;
		vector<int> &vars = _netvar2satvar[var];
		...
		int res = deduce();
//		if (res == CONFLICT) {
//			learn_conflict(); // one level clause learning (by Radu)
//		}

		return res;
	}

	void CSolver::learn_conflict() {
		assert(!_conflicts.empty());
		...
		conflict_analysis_firstUIP_one_step();
	}

	// Propagate a list of variable-value assignments from the master problem.
	int CSolver::propagate(std::list<pair<int,int> > &assignments) {
		int vid;
		std::list<std::pair<int,int> >::iterator li = assignments.begin();
		for (; li != assignments.end(); ++li)
		{
			int var = (*li).first;
			int val = (*li).second;
			vector<int> &vars = _netvar2satvar[var];
	...
	void CSolver::assert_assignments(std::list<std::pair<int,int> > &assignments) {
		int vid;
		std::list<std::pair<int,int> >::iterator li = assignments.begin();
		for (; li != assignments.end(); ++li)
		{
	...
	int CSolver::deduce(void) {
		_conflicts.clear();	// when doing clause learning, remove the line.
		while (!_implication_queue.empty()) {
			...
			_implication_queue.pop();
			CVariable & var = variable(vid);
			if (var.value() == ZCHAFF_UNKNOWN) {  // an implication
				...
				if (var.antecedent() != NULL_CLAUSE && cl != NULL_CLAUSE &&
	...
	void CSolver::mark_vars(ClauseIdx cl, int var_idx) {
		...
		for (CLitPoolElement* itr = clause(cl).literals(); (*itr).val() > 0; ++itr) {
			...
			if (v == var_idx)
				...
				if (variable(v).new_cl_phase() == ZCHAFF_UNKNOWN) {  // it's not in the new cl
	...
	int CSolver::finish_add_conf_clause(int gflag) {
		...
		//adjust_variable_order(&(*_conflict_lits.begin()), (int)_conflict_lits.size());

		if (_params.shrinking.enable) {
			_shrinking_cls.clear();
			if (_stats.shrinking_cls_length != 0) {
				int benefit = _stats.shrinking_cls_length - (int)_conflict_lits.size();
			...
			if (_conflict_lits.size() > _params.shrinking.size) {
				_shrinking_cls.clear();
				for (unsigned i = 0, sz = (unsigned)_conflict_lits.size(); i < sz; ++i) {
				...
				if (found_gap && _shrinking_cls.size() > 0 && prev_dl < dlevel() - 1) {
					_stats.shrinking_cls_length = (int)(_conflict_lits.size());
					...
					_num_in_new_cl = 0;
					for (unsigned i = 0, sz = (unsigned)_conflict_lits.size(); i < sz; ++i)
						variable(_conflict_lits[i]>>1).set_new_cl_phase(ZCHAFF_UNKNOWN);
		...
		for (unsigned i = 0; i < clause(added_cl).num_lits(); ++i) {
			...
			assert(variable(vid).value() != ZCHAFF_UNKNOWN);
		...
		while (!_conflict_lits.empty()) {
			...
			--_num_in_new_cl;
			var.set_new_cl_phase(ZCHAFF_UNKNOWN);
		}
		assert(_num_in_new_cl == 0);
		return back_dl;
	}

	int CSolver::finish_add_conf_clause_one_step(int gflag) {
		ClauseIdx added_cl = add_conflict_clause(&(*_conflict_lits.begin()),
			(int)_conflict_lits.size(), gflag);
		if (added_cl < 0) {  // memory out.
			_stats.is_mem_out = true;
				...
				ci != _conflicts.end(); ci++) {
					...
					for (int i = (int)assignments.size() - 1; i >= 0; --i) {
						...
						if (variable(assigned >> 1).is_marked()) {
							...
							if ( _num_marked == 0 ) {
								...
								assert(variable(assigned>>1).new_cl_phase() == ZCHAFF_UNKNOWN);
					...
					if (min_conf_length == -1 ||
						(int)_conflict_lits.size() < min_conf_length) {
							min_conf_length = (int)(_conflict_lits.size());
							min_conf_id = cl;
						}
	...
	int CSolver::conflict_analysis_firstUIP_one_step(void) {
		int min_conf_id = _conflicts[0];
		int min_conf_length = -1;
		ClauseIdx cl;
		unsigned gflag;
		...
		return finish_add_conf_clause_one_step(gflag);
	}
	...
	int CSolver::mem_usage(void) {
		...
		for (int i = 0; i < _stats.max_dlevel; ++i)
			mem_assignment += (int)(_assignment_stack[i]->capacity() * sizeof(int));
		mem_assignment += (int)(sizeof(vector<int>)* _assignment_stack.size());
