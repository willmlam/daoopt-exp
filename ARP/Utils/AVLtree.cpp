...
#ifdef DEBUG_AVL_new
CMauiAVLTree *_msTestAVLTree = NULL ;
#endif // DEBUG_AVL_new
...
/*
	********** ********** ********** ********** ********** ********** ********** ********** **********
	CMauiAVLTree member functions.
	********** ********** ********** ********** ********** ********** ********** ********** **********
*/

void MauiAVLNode::operator<<(const _MauiAVLNode & Node)
{
	m_id = Node.m_id ;
...
void CMauiAVLTree::SetReallocationSize(unsigned long reallocation_size)
{
...
int CMauiAVLTree::AllocateMemory(unsigned long new_size)
{
	unsigned long i ;
	...
	if (new_size > 0) 
		m_db_tree = new MauiAVLNode[new_size] ;
	else 
		return 1 ;
	if (NULL == m_db_tree) {
		m_allocated_space = 0 ;
...
int CMauiAVLTree::ReAllocateMemory(void)
{
	unsigned long new_size, i ;
	MauiAVLNode *new_db_tree = NULL ;
	...
	new_db_tree = new MauiAVLNode[new_size] ;
	if (NULL == new_db_tree) return 0 ;

	// copy the entire old tree into the new tree
	memcpy(new_db_tree,m_db_tree,(size_t) i*sizeof(MauiAVLNode)) ;
...
CMauiAVLTree::CMauiAVLTree(void)
{
	m_db_tree = NULL ;
	m_allocated_space = 0 ;
	m_size = 0 ;
...
//	if (! AllocateMemory(AVL_DEFAULT_SIZE)) 
//		return ;
}
...
CMauiAVLTree::CMauiAVLTree(unsigned long tree_size /* number of keys (>= 0) */, 
...
/*
	Qualification check.
*/
	if (tree_size > 0 && (NULL == key || NULL == data)) tree_size = 0 ;
	if (tree_size >= AVL_SIZE_LIMIT) 
		// if too many keys. cut it short.
	...
	if (space > 0) {
		...
		// Allocate memory for the AVL tree. 
 ...
 * ********** ********** */
	...
	if (tree_size > 0) {
		...
		QuickSortLong(key,tree_size,data,left,right) ;
		}
...
*/
	...
	for (i = 0 ; i < tree_size ; i++) {
		...
		m_db_tree[m_size].m_data = data[i] ;
...
void CMauiAVLTree::PrintTree(void)
{
	// TODO.
}
...
long CMauiAVLTree::CheckTree(char **pErrorStr)
{
	unsigned long i, j, m ;
	unsigned long k ;
...
void CMauiAVLTree::R_rotation(unsigned long k)
...
void CMauiAVLTree::L_rotation(unsigned long k)
...
void CMauiAVLTree::BalanceInsertTree(void)
// balance the tree
...
void CMauiAVLTree::BalanceDeleteTree(void)
...
long CMauiAVLTree::Find(long id, long *data /* return data here */)  
{
#ifdef DEBUG_AVL_mutex
...
down_the_tree_12_04_94:
	...
	if (m_db_tree[j].m_id == id) {
		Left[i] = 0 ;
		path_len = i ;
		if (data) *data = m_db_tree[j].m_data ;
...
long CMauiAVLTree::FindNext(long key, long *next_key, long *next_data /* return data here */)  
{
	long j ;
	if ((j = Find(key, NULL))) {
		...
		if (0 == j) {
			...
			if (next_data) *next_data = 0 ;
			}
		else {
			if (next_key) *next_key = m_db_tree[j].m_id ;
			if (next_data) *next_data = m_db_tree[j].m_data ;
			}
		return 1 ;
		}
...
long CMauiAVLTree::Increment(long id)
{
	long j ;
...
long CMauiAVLTree::Adjust(long id, long data_adjustment, bool OnlyIfExists)
{
	long j ;
	...
	if ((j = Find(id, NULL))) {
		// this entity is already in the database; adjust data.
		m_db_tree[j].m_data += data_adjustment ;
		return 1 ;
		}
	...
	// insert with data
	return Insert(id, data_adjustment) ;
}
...
long CMauiAVLTree::Decrement(long id, int remove_if_0)
{
	long j ;
	...
	if ((j = Find(id, NULL))) {
		if (0 == --m_db_tree[j].m_data) {
			if (remove_if_0) {
				Remove(id, NULL) ;
				return -1 ;
				}
			}
		return m_db_tree[j].m_data ;
		}
...
int CMauiAVLTree::SetData(unsigned long idx, long data)
{
	if (idx < 1 || idx > m_allocated_space) 
		return 0 ;
	m_db_tree[idx].m_data = data ;
	return 1 ;
}
...
long CMauiAVLTree::Insert(long id, long data)
{
#ifdef DEBUG_AVL_mutex
	if (NULL == _msAVLdebugMutex) 
		_msAVLdebugMutex = new COptexEXT_debug("AVLtestMutex") ;
...
*/
	...
	if (NULL == _msTestAVLTree) {
		try {
			_msTestAVLTree = new CMauiAVLTree ;
			}
		catch (...) {
			}
		}
...
long CMauiAVLTree::RemoveFirst(long *key /* output */, long *data /* return data here. can be NULL */) // returns TRUE iff everything is fine
{
#ifdef DEBUG_AVL_mutex
	if (NULL == _msAVLdebugMutex) 
		_msAVLdebugMutex = new COptexEXT_debug("AVLtestMutex") ;
...
long CMauiAVLTree::Remove(long id, long *data /* return data here */)
{
#ifdef DEBUG_AVL_mutex
	if (NULL == _msAVLdebugMutex) 
		_msAVLdebugMutex = new COptexEXT_debug("AVLtestMutex") ;
...
//long CMauiAVLTree::get_first_key_data(long *key /* can be NULL */, long *data /* can be NULL */)
//{
...
//long CMauiAVLTree::get_second_key_data(long *key /* can be NULL */, long *data /* can be NULL */)
//{
...
long CMauiAVLTree::GetFirst(long *key /* can be NULL */, long *data /* can be NULL */, long & current) const 
{
	if (m_size < 1) {
		current = 0 ;
		return 0 ;
	...
	if (data) *data = m_db_tree[current].m_data ;
	return 1 ;
}
...
long CMauiAVLTree::GetNext(long *key /* can be NULL */, long *data /* can be NULL */, long & current) const 
{
...
long CMauiAVLTree::GetNextAndSetData(long *key /* can be NULL */, long *data /* can be NULL */, long NewData, long & current) const 
{
	// if the tree is empty, cannot select anything
	if (m_size < 1) {
		if (key) *key = 0 ;
	...
	if (current < 0) {
		...
		if (key) *key = m_db_tree[current].m_id ;
		if (data) *data = m_db_tree[current].m_data ;
		m_db_tree[current].m_data = NewData ;
		return 1 ;
		}
	if (0 == current || current > (long)m_allocated_space) {
		if (key) *key = 0 ;
...
long CMauiAVLTree::GetCurrentAndAdvance(long *key /* can be NULL */, long *data /* can be NULL */, long & current) const 
{
	// if the tree is empty, cannot select anything
	if (m_size < 1) {
		if (key) *key = 0 ;
...
long CMauiAVLTree::FindSmallestPositiveKeyNotUsed(void)
{
	long i, j, target_key ;
...
int CMauiAVLTree::FindData(long data)  
{
	long i ;
	...
	if (m_size < 1) return 1 ;
	int count = 0 ;
	for (i = m_first_used_block ; i > 0 ; i = m_db_tree[i].m_next_entity) {
		if (m_db_tree[i].m_data == data) 
			count++ ;
		}
...
int CMauiAVLTree::DecrementLargerKeys(long key)
{
	long i, j, start_entity ;

	// check if empty
	if (m_size < 1) return 1 ;

	if ((j = Find(key,&i))) {
		// this key is in the database
		start_entity = m_db_tree[j].m_next_entity ;

		// check if key+1 is also in the database
		if (m_db_tree[start_entity].m_id < key + 2) return 0 ;
		}
	else {
		// now, find the right entity to start from

		if (Left[path_len] == 1) { // right son
			// parent key is smaller, cannot take it
			start_entity = Left[--path_len] ; // the parent
			// however, the next from parent has to be larger than the given key
			start_entity = m_db_tree[start_entity].m_next_entity ;
			}
		else { // left son, ie. the parent key is larger, keep it
			start_entity = Left[--path_len] ; // the parent
			}
		}

	for (i = start_entity ; i > 0 ; i = m_db_tree[i].m_next_entity) {
		--(m_db_tree[i].m_id) ;
		}
...
long CMauiAVLTree::TestTree(int numIterations, char **pErrorStr) // returns TRUE iff the tree is OK.
{
	if (pErrorStr) 
		*pErrorStr = NULL ;
	if (numIterations < 1) 
...
bool CMauiAVLTree::operator<<(const CMauiAVLTree & AVLtree)
{
	unsigned long i ;
...
bool CMauiAVLTree::TestConsistency(long checkbitvector) const
{
