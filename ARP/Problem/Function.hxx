...
#include "Problem/FunctionTableBlock.hxx"
...
protected :
	...
	int *_ArgumentsPermutationList ; // this is a list of indeces of the arguments of this fn wrt some other ordering of variables. if is allocated when _Arguments is allocated.
public :
	...
	inline int *ArgumentsPermutationList(void) { return _ArgumentsPermutationList ; }
	...
	void ComputeArgumentsPermutationList(int n, const int *vars)
	{
	...
	inline __int64 ComputeFnTableAdr_wrtLocalPermutation(int NSuperSet, const int *ValSuperSet, const int *DomainSizes)
	{
		if (_nArgs < 1) 
			return -1 ;
		if (NSuperSet < _nArgs) 
			return 0 ;
		int i = _nArgs - 1 ;
		__int64 adr = ValSuperSet[_ArgumentsPermutationList[i]] ;
		__int64 j = 1 ;
		for (--i ; i >= 0 ; i--) {
...
public :
	...
	double Compute0Tightness(void)
	{
		__int64 n0s = 0 ;
		for (FunctionTableBlock *ftb = _Table ; NULL != ftb ; ftb = ftb->NextFTBInFunction()) 
			n0s += ftb->CountNumberOf0s() ;
		return _TableSize > 0 ? ((double)n0s)/((double)_TableSize) : -1.0 ;
	}

	int ConvertTableToLogScale(void) ;
...
//	int FactorizeArgumentDomains(void) ;
	...
	int AllocateInMemoryAsSingleTableBlock(void) ;
