...
int ARE::fileload_getnexttoken(const char * & buf, int & L, const char * & B, int & l, bool IncludeSpecialSymbols)
{
	B = NULL ;
	l = 0 ;
	// find beginning
...
int ARE::ARP::LoadFromBuffer(const char *format, const char *buf, int L)
{
	int i ;
...
int ARE::ARP::LoadFromBuffer_Evidence(const char *format, const char *buf, int L, int & nEvidenceVars)
{
	nEvidenceVars = 0 ;
...
//#define bufsize	33554432
	...
	int res = LoadFromBuffer("UAI", buf, L) ;
	delete [] buf ;
	return res ;
}
...
//			continue ;
	...
	for (i = 0 ; i < _nFunctions ; i++) {
		ARE::Function *f = _Functions[i] ;
		if (NULL == f) continue ;
		if (f->TableSize() <= 0) continue ; // do data to load
		ARE::FunctionTableBlock *ftb = f->Table() ;
...
int ARE::ARP::LoadFromFile_Evidence(const std::string & FileName, int & nEvidenceVars)
{
	nEvidenceVars = 0 ;
...
int ARE::ARP::LoadUAIFormat_Evidence(const char *BUF, int L, int & nEvidenceVars)
{
...
int ARE::ARP::DestroyAdjFnList(void)
{
...
int ARE::ARP::ComputeAdjFnList(bool IgnoreIrrelevantFunctions)
{
	...
	_nAdjFunctions = new int[_nVars] ;
	_AdjFunctions = new int[_nVars] ;
...
int ARE::ARP::DestroyAdjVarList(void)
{
...
int ARE::ARP::ComputeAdjVarList(void)
{
	...
	_Degree = new int[_nVars] ;
	_AdjVars = new int[_nVars] ;
	_StaticVarTotalList = UBvarlistsize > 0 ? new int[UBvarlistsize] : NULL ;
	...
	long *vars = new long[UBvarlistsize] ;
	if (NULL == vars) {
		DestroyAdjVarList() ;
		return 1 ;
		}
	long left[32], right[32] ;
	int idxStaticVarTotalList = 0 ;
	for (i = 0 ; i < _nVars ; i++) {
		unsigned long n = 0 ;
...
int ARE::ARP::GetVarElimOrdering(std::vector<int> & Order, int & induced_width)
{
	Order.clear() ;
	induced_width = -1 ;
	if (NULL == _VarOrdering_VarList) 
...
int ARE::ARP::SetVarElimOrdering(const int *VarListInElimOrder, int induced_width)
{
	if (_nVars <= 0) {
		DestroyVarOrdering() ;
		return 0 ;
...
int ARE::ARP::SetVarBTOrdering(const int *VarListInBTOrder, int induced_width)
{
	if (_nVars <= 0) {
		DestroyVarOrdering() ;
		return 0 ;
...
int ARE::ARP::RemoveAdjVar(int U, int V)
{
	int *adjlist = _StaticVarTotalList + _AdjVars[U] ;
	for (int i = _Degree[U]-1 ; i >= 0 ; i--) {
		if (V == adjlist[i]) {
...
int ARE::ARP::EliminateEvidence(void)
{
	int i, nEvidVars = 0 ;
	for (i = 0 ; i < N() ; i++) {
		int val = Value(i) ;
