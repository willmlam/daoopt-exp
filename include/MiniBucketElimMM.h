...
class MiniBucketElimMM : public Heuristic {
...
public:
  MiniBucketElimMM(Problem* p, Pseudotree* pt, ProgramOptions* po, int ib);
  virtual ~MiniBucketElimMM();

};

/* Inline definitions */

inline bool MiniBucketElimMM::isAccurate() {
  assert(m_pseudotree);
  return (m_pseudotree->getWidthCond() <= m_ibound);
}

inline MiniBucketElimMM::MiniBucketElimMM(Problem* p, Pseudotree* pt,
				      ProgramOptions* po, int ib) :
    Heuristic(p, pt, po), m_ibound(ib), m_globalUB(ELEM_ONE)
// , m_augmented(p->getN()), m_intermediate(p->getN())
  { }

inline MiniBucketElimMM::~MiniBucketElimMM() {
